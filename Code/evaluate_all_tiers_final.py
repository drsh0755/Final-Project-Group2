...
RESULTS_DIR = BASE_DIR / "Results" / "evaluation_final"
...
print("=" * 60)
print("FINAL EVALUATION — ALL TIERS ALL VERSIONS (Tier1/2 v1+v2, Tier3 v1+v2+v3a+v3b)")
print("=" * 60)
...
def tier1_translate_poem_v1(poem: str) -> str:
    """Original v1 — compact dictionary, bracket unknown tokens."""
    D = {
        "मैं": "I", "मुझे": "me", "मेरा": "my", "हम": "we",
        "तू": "you", "तुम": "you", "तेरा": "your", "वह": "he/she",
        "वो": "he/she", "यह": "this", "वे": "they",
        "कोई": "someone", "कुछ": "something", "सब": "all", "हर": "every",
        "है": "is", "हैं": "are", "था": "was", "थी": "was", "थे": "were",
        "हो": "be", "हूँ": "am", "हुआ": "became",
        "जाना": "to go", "जाता": "goes", "जा": "go",
        "आना": "to come", "आता": "comes", "आ": "come",
        "करना": "to do", "करता": "does", "कर": "do",
        "देना": "to give", "दिया": "gave", "कहना": "to say", "कहा": "said",
        "देखना": "to see", "देखा": "saw", "रहना": "to stay",
        "जीना": "to live", "मरना": "to die",
        "खोना": "to lose", "खोया": "lost", "याद": "memory",
        "दिल": "heart", "प्यार": "love", "इश्क": "love",
        "दर्द": "pain", "गम": "sorrow", "ख़ुशी": "happiness",
        "आँसू": "tears", "उम्मीद": "hope", "ख़्वाब": "dream",
        "डर": "fear", "तड़प": "longing", "आरज़ू": "desire", "सुकून": "peace",
        "रात": "night", "दिन": "day", "चाँद": "moon", "सूरज": "sun",
        "आसमान": "sky", "तारे": "stars", "बादल": "cloud",
        "बारिश": "rain", "हवा": "wind", "फूल": "flower", "पेड़": "tree",
        "रोशनी": "light", "अँधेरा": "darkness", "आग": "fire",
        "शाम": "evening", "सुबह": "morning",
        "आँखें": "eyes", "होंठ": "lips", "हाथ": "hand",
        "मन": "mind", "रूह": "soul", "जिंदगी": "life", "ज़िंदगी": "life",
        "मौत": "death", "साँस": "breath",
        "सुंदर": "beautiful", "अच्छा": "good", "बुरा": "bad",
        "में": "in", "पर": "on", "से": "from", "को": "to",
        "के": "of", "की": "of", "का": "of", "साथ": "with",
        "लिए": "for", "बिना": "without", "अब": "now", "फिर": "again",
        "नहीं": "not", "ना": "no", "और": "and", "या": "or",
        "लेकिन": "but", "भी": "also",
        "शायर": "poet", "ग़ज़ल": "ghazal", "दीवाना": "madman/lover",
        "मंजिल": "destination", "राह": "path",
        "मुसाफिर": "traveler", "सफर": "journey",
        "वक्त": "time", "दुनिया": "world", "खुदा": "God", "दुआ": "prayer",
    }
    import re
    lines = [l.strip() for l in poem.split("\n") if l.strip()]
    translated = []
    for line in lines:
        tokens = re.sub(r'[।\.!?,;:\-"\']+', ' ', line).split()
        words = [D.get(t, D.get(t.lower(), f"[{t}]")) for t in tokens]
        translated.append(" ".join(words))
    return "\n".join(translated)
...
def load_tier2_model(folder="tier2_seq2seq_v2"):
    ckpt_path  = MODELS_DIR / folder / "best_model.pt"
    vocab_path = MODELS_DIR / folder / "vocabs.pt"
...
def load_tier3_model(version="v3a"):
    from transformers import MarianMTModel, MarianTokenizer
    model_path = MODELS_DIR / "tier3_opusmt" / f"tier3_{version}"
    if not model_path.exists():
        print(f"  ⚠️  Tier 3 {version} model not found at {model_path}")
        return None, None
    tokenizer = MarianTokenizer.from_pretrained(str(model_path))
    model     = MarianMTModel.from_pretrained(str(model_path)).to(DEVICE)
    model.eval()
    print(f"  Tier 3 {version} loaded from {model_path}")
...
if __name__ == "__main__":
    ...
    print("Models: Tier1 v1+v2 | Tier2 v1+v2 | Tier3 v1+v2+v3a+v3b\n")

    all_results = []

    # ── Tier 1 v1 ─────────────────────────────────────────────────────────────
    print("─" * 50)
    print("Tier 1 v1 — Rule-Based (basic dictionary)...")
    t1v1_results = evaluate_tier("Tier1_RuleBased_v1", tier1_translate_poem_v1, poems_df)
    all_results.extend(t1v1_results)

    # ── Tier 1 v2 ─────────────────────────────────────────────────────────────
    print("\nTier 1 v2 — Rule-Based (expanded dictionary + transliteration)...")
    t1v2_results = evaluate_tier("Tier1_RuleBased_v2", tier1_translate_poem, poems_df)
    all_results.extend(t1v2_results)

    # ── Tier 2 v1 ─────────────────────────────────────────────────────────────
    print("\nLoading Tier 2 v1 (Seq2Seq LSTM)...")
    t2v1_model, t2v1_vocab, _ = load_tier2_model("tier2_seq2seq")
    if t2v1_model:
        t2v1_fn = lambda poem: tier2_translate_poem(t2v1_model, t2v1_vocab, poem)
        t2v1_results = evaluate_tier("Tier2_Seq2Seq_v1", t2v1_fn, poems_df)
        all_results.extend(t2v1_results)
        del t2v1_model; torch.cuda.empty_cache()
    else:
        t2v1_results = []

    # ── Tier 2 v2 ─────────────────────────────────────────────────────────────
    print("\nLoading Tier 2 v2 (Seq2Seq LSTM augmented)...")
    t2v2_model, t2v2_vocab, _ = load_tier2_model("tier2_seq2seq_v2")
    if t2v2_model:
        t2v2_fn = lambda poem: tier2_translate_poem(t2v2_model, t2v2_vocab, poem)
        t2v2_results = evaluate_tier("Tier2_Seq2Seq_v2", t2v2_fn, poems_df)
        all_results.extend(t2v2_results)
        del t2v2_model; torch.cuda.empty_cache()
    else:
        t2v2_results = []

    # ── Tier 3 v1 ─────────────────────────────────────────────────────────────
    print("\nLoading Tier 3 v1 (Opus-MT baseline)...")
    t3v1_model, t3v1_tokenizer = load_tier3_model("v1")
    if t3v1_model:
        t3v1_fn = lambda poem: tier3_translate_poem(t3v1_model, t3v1_tokenizer, poem)
        t3v1_results = evaluate_tier("Tier3_OpusMT_v1", t3v1_fn, poems_df)
        all_results.extend(t3v1_results)
        del t3v1_model; torch.cuda.empty_cache()
    else:
        t3v1_results = []

    # ── Tier 3 v2 ─────────────────────────────────────────────────────────────
    print("\nLoading Tier 3 v2 (Opus-MT improved)...")
    t3v2_model, t3v2_tokenizer = load_tier3_model("v2")
    if t3v2_model:
        t3v2_fn = lambda poem: tier3_translate_poem(t3v2_model, t3v2_tokenizer, poem)
        t3v2_results = evaluate_tier("Tier3_OpusMT_v2", t3v2_fn, poems_df)
        all_results.extend(t3v2_results)
        del t3v2_model; torch.cuda.empty_cache()
    else:
        t3v2_results = []

    # ── Tier 3 v3a ────────────────────────────────────────────────────────────
    print("\nLoading Tier 3 v3a (Interleaved)...")
    t3a_model, t3a_tokenizer = load_tier3_model("v3a")
    if t3a_model:
        t3a_fn = lambda poem: tier3_translate_poem(t3a_model, t3a_tokenizer, poem)
        t3a_results = evaluate_tier("Tier3_OpusMT_v3a", t3a_fn, poems_df)
        all_results.extend(t3a_results)
        del t3a_model; torch.cuda.empty_cache()
    else:
        t3a_results = []

    # ── Tier 3 v3b ────────────────────────────────────────────────────────────
    print("\nLoading Tier 3 v3b (Curriculum)...")
    t3b_model, t3b_tokenizer = load_tier3_model("v3b")
    if t3b_model:
        t3b_fn = lambda poem: tier3_translate_poem(t3b_model, t3b_tokenizer, poem)
        t3b_results = evaluate_tier("Tier3_OpusMT_v3b", t3b_fn, poems_df)
        all_results.extend(t3b_results)
        del t3b_model; torch.cuda.empty_cache()
    else:
        t3b_results = []
    ...
    summary = [s for s in [
        summarize(t1v1_results,  "Tier1_RuleBased_v1"),
        summarize(t1v2_results,  "Tier1_RuleBased_v2"),
        summarize(t2v1_results,  "Tier2_Seq2Seq_v1"),
        summarize(t2v2_results,  "Tier2_Seq2Seq_v2"),
        summarize(t3v1_results,  "Tier3_OpusMT_v1"),
        summarize(t3v2_results,  "Tier3_OpusMT_v2"),
        summarize(t3a_results,   "Tier3_OpusMT_v3a"),
        summarize(t3b_results,   "Tier3_OpusMT_v3b"),
    ] if s]
